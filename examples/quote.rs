    ...
    rx.iter().for_each(|ev| match ev {
        Event::Quote(q) => {
            ...
            log::info!(
                ...
                q.Volume,q.Turnover,
                if q.OpenPrice.is_infinite() {q.OpenPrice}else {f64::NAN},if q.HighestPrice.is_infinite() {q.HighestPrice}else {f64::NAN}, if q.LowestPrice.is_normal() {q.LowestPrice}else {f64::NAN},if q.ClosePrice.is_infinite() {q.ClosePrice}else {f64::NAN},
                if q.AskPrice1.is_infinite() {q.AskPrice1}else {f64::NAN},
                q.AskVolume1,
                if q.BidPrice1.is_infinite() {q.BidPrice1}else {f64::NAN},
                q.BidVolume1,
                if q.AskPrice2.is_infinite() {q.AskPrice2}else {f64::NAN},
                q.AskVolume2,
                if q.BidPrice2.is_infinite() {q.BidPrice2}else {f64::NAN},
                q.BidVolume2,
                if q.AskPrice3.is_infinite() {q.AskPrice3}else {f64::NAN},
                q.AskVolume3,
                if q.BidPrice3.is_infinite() {q.BidPrice3}else {f64::NAN},
                q.BidVolume3,
                if q.AskPrice4.is_infinite() {q.AskPrice4}else {f64::NAN},
                q.AskVolume4,
                if q.BidPrice4.is_infinite() {q.BidPrice4}else {f64::NAN},
                q.BidVolume4,
                if q.AskPrice5.is_infinite() {q.AskPrice5}else {f64::NAN},
                q.AskVolume5,
                if q.BidPrice5.is_infinite() {q.BidPrice5}else {f64::NAN},
                q.BidVolume5,
            );
        }
        Event::Login(login) => {
